 ...
 * 状态转换的目标状态构建器接口
 * <p>
 * 在状态机的构建过程中，此接口用于指定触发状态转换的事件。
 * 它是构建状态转换链中的第三步，在指定源状态(from)和目标状态(to)之后使用。
 * <p>
 * 泛型参数说明：
 * <ul>
 ...
 *     .when(context -> context.isValid()) // 添加条件
...
public interface GXTo<S, E, C> {
     ...
     * 指定触发状态转换的事件
     * <p>
     * 此方法定义了哪个事件会触发这个状态转换，是构建状态转换的第三步
     *
     * @param event 触发转换的事件，类型为E（由使用者定义的事件类型）
     * @return 返回On子句构建器，用于继续构建转换链，可以添加条件或动作
     */
