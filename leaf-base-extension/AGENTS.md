...
11. 所有修改应优先保持现有对外行为不变；若行为变化不可避免，必须同步补充测试和说明。
...
8. 本次改动是否只限制在 `leaf-base-extension` 模块内。
9. 对外接口规范是否保持兼容。
10. 性能与线程安全风险是否已评估并通过测试覆盖。
11. 适合虚拟线程的场景是否优先采用虚拟线程。
12. 已有逻辑的正确性是否保持不变。
13. 是否严格遵循测试驱动最佳实践，先补充测试再实现代码变更。
