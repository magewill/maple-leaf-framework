...
public class GXAsyncConfig implements AsyncConfigurer {
     ...
     * 默认任务超时时间(毫秒)
     */
    private static final long DEFAULT_TASK_TIMEOUT_MS = 60000;
    ...
    public Executor getAsyncExecutor() {
        ...
        // 计算核心线程数和最大线程数
        int corePoolSize = Math.max(1, (int) Math.ceil(cpuCores * corePoolSizeFactor));
        int maxPoolSize = Math.max(corePoolSize, (int) Math.ceil(corePoolSize * maxPoolSizeFactor));
    ...
    private RejectedExecutionHandler createCustomRejectedExecutionHandler() {
        return (runnable, executor) -> {
            ...
            log.error("任务被拒绝: 线程池饱和. 状态: [活动线程数={}, 队列大小={}, 当前池大小={}, 核心池大小={}, 最大池大小={}, 系统负载={}]",
                    ...
                    systemLoad >= 0 ? String.format("%.2f", systemLoad) : "N/A");

            // 系统负载较高或线程池已达最大值时直接拒绝,防止系统崩溃
            if ((systemLoad >= 0 && systemLoad > highLoadThreshold) || activeCount >= maximumPoolSize) {
    ...
    public AsyncUncaughtExceptionHandler getAsyncUncaughtExceptionHandler() {
        return GXSpringContextUtils.getBean(GXAsyncExceptionHandler.class);
    }
