...
8. 在确保原有功能正确性的前提下进行完善；修改已有逻辑时，不得改变现有逻辑的正确性和既有业务语义。
9. 作为基础框架模块，代码变更必须重点关注性能、资源释放、并发可见性和线程安全性。
10. 在线程池与虚拟线程都能满足需求的场景中，优先使用虚拟线程；若继续使用线程池，必须说明容量、生命周期和拒绝策略的合理性。
11. 变更连接配置解析逻辑（单机/集群判断、地址解码、线程参数默认值）前，必须评估兼容性影响。
12. 变更 MQ 监听注册与注销逻辑（本地缓存键、重复订阅控制、销毁阶段清理）前，必须评估消息重复消费和监听泄漏风险。
...
10. 是否确认本次完善没有改变已有逻辑的正确性和既有业务语义。
11. 是否评估了性能、资源释放和线程安全风险。
12. 是否在适用场景优先考虑虚拟线程。
