 ...
 * Feign认证Token切面
 * <p>
 * 该切面负责拦截标记了@GXFeignAuthToken注解的方法或类，并在方法执行前验证Feign的认证Token是否有效。
 * 如果Token无效，将抛出GXFeignAuthTokenException异常阻止方法执行，保障API的安全性。
 * </p>
 *
 * <p>
 * 工作流程：
 * 1. 定义切点，匹配标记了@GXFeignAuthToken注解的方法或类
 * 2. 在方法执行前（@Before），获取GXFeignService实例
 * 3. 调用GXFeignService的checkTokenValidity方法验证Token
 ...
 * 使用示例：
 * <pre>
 * // 1. 首先实现GXFeignService接口
 * @Service
 * public class CustomFeignService implements GXFeignService {
 *     // 使用线程安全的缓存存储Token
 *     private final LoadingCache<String, String> tokenCache;
 * <p>
 *     public CustomFeignService() {
 *         // 创建带有过期时间的缓存
 ...
 *         return tokenCache.get("FeignToken");
 *     }
 *
 *     @Override
 ...
 * }
 * <p>
 * // 2. 在类级别应用@GXFeignAuthToken注解，类中所有方法都会进行Token验证
 * @GXFeignAuthToken
 * @RestController
 ...
 * }
 * <p>
 * // 3. 在方法级别应用@GXFeignAuthToken注解，只对特定方法进行Token验证
 * @RestController
 ...
 * 3. 依赖的GXFeignService实现类应确保其checkTokenValidity方法是线程安全的
 * 4. 在高并发环境下，应避免在checkTokenValidity方法中使用同步块，可能导致性能瓶颈
 * 5. 如需在GXFeignService实现类中缓存Token信息，应使用线程安全的缓存实现，如ConcurrentHashMap或Caffeine
 * </p>
 *
 * <p>
 * 性能优化：
 ...
 * <p>
 * 扩展建议：
 * 1. 可扩展@GXFeignAuthToken注解，支持配置不同的验证策略或安全级别
...
public class GXFeignAuthTokenAspect {
    ...
    /**
     * 定义切点，匹配标记了@GXFeignAuthToken注解的方法或类
     * <p>
     * 切点表达式说明：
     ...
     * <p>
     * 该切点将捕获以下两种情况：
     * 1. 直接在方法上标记了@GXFeignAuthToken注解的方法
     * 2. 在类上标记了@GXFeignAuthToken注解的类中的所有方法
     * </p>
     * <p>
     * 性能说明：
    ...
    /**
     * 在目标方法执行前验证Feign认证Token
     * <p>
     * 该方法在匹配切点的方法执行前被调用，负责验证Feign的认证Token是否有效。
     * 验证流程：
     * 1. 从Spring容器中获取GXFeignService实例
     * 2. 获取当前方法的签名信息，用于日志记录和异常信息
     * 3. 调用GXFeignService.checkTokenValidity()方法验证Token
     * 4. 如果Token无效，记录错误日志并抛出GXFeignAuthTokenException异常
     * </p>
     * <p>
     * 异常处理：
     * - 如果未找到GXFeignService实现类，抛出GXBusinessException异常
     * - 如果Token验证失败，抛出GXFeignAuthTokenException异常
     * </p>
     * <p>
     * 日志记录：
     * - 验证开始时记录debug级别日志
    ...
    public void before(JoinPoint point) {
        ...
        log.debug("正在验证Feign调用token，方法: {}", methodName);
        ...
        log.debug("Feign调用token校验成功，方法: {}", methodName);
    }
